...
import { getJwtConfig, verifyAccessToken } from '../lib/jwt.js';
...
async function adminAuthMiddleware(request: FastifyRequest, reply: FastifyReply) {
    ...
    try {
        ...
        // For admin portal, we just verify the token is valid
        // In production, you may want to add an AdminUser table or check specific admin claims
        if (!decoded.sub) {
            return reply.status(401).send({
                success: false,
                error: {
    ...
    fastify.get<{
        ...
        async (request, reply) => {
            ...
            try {
                ...
                // Transform response
                const items = subscribers.map((tenant) => ({
                    id: tenant.id,
                    name: tenant.name,
                    slug: tenant.slug,
                    tier: tenant.tier,
    ...
    fastify.get<{
        ...
        async (request, reply) => {
            ...
            try {
                ...
                const activeLicense = tenant.licenses.find(l => l.isActive);
                ...
                return reply.send({
                    ...
                    data: {
                        ...
                        allLicenses: tenant.licenses.map(l => ({
                            id: l.id,
