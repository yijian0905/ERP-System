...
export async function dashboardRoutes(fastify: FastifyInstance) {
    ...
    fastify.get('/stats', async (request: FastifyRequest, reply: FastifyReply) => {
        try {
            ...
            const inventoryValue = inventoryItems.reduce((sum: number, item: { quantity: number; costPrice: { toNumber: () => number } | null }) => {
    ...
    fastify.get('/low-stock', async (request: FastifyRequest, reply: FastifyReply) => {
        try {
            ...
            const lowStockItems = inventoryItems
                .filter((item: { quantity: number; product: { reorderPoint: number } }) =>
            ...
            const products = lowStockItems.map((item: {
                quantity: number;
                product: { sku: string; name: string; reorderPoint: number };
            }) => ({
