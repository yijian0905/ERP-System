...
import { LHDN_DOCUMENT_TYPE_CODES } from '@erp/shared-types';
...
export class EInvoiceBuilder {
  ...
  private supplierInfo: SupplierInfo | null = null;
  ...
  /**
   * Load supplier info from LHDN credentials
   */
  private async loadSupplierInfo(): Promise<SupplierInfo> {
    ...
    this.supplierInfo = {
      ...
      address: {
        street: companyInfo.address as string | undefined,
      ...
      industryCode: companyInfo.industryCode as string | undefined,
    };
  ...
  /**
   * Build supplier party section
   */
  private buildSupplierParty(supplier: SupplierInfo) {
    return {
      Party: [{
        IndustryClassificationCode: supplier.industryCode ? [{
          _: supplier.industryCode,
          listVersionID: '1.0',
        }] : undefined,
        PartyIdentification: [
          {
        ...
        PostalAddress: [{
          CityName: supplier.address.city ? [{ _: supplier.address.city }] : undefined,
          PostalZone: supplier.address.postalCode ? [{ _: supplier.address.postalCode }] : undefined,
          CountrySubentityCode: supplier.address.state ? [{ _: supplier.address.state }] : undefined,
          AddressLine: supplier.address.street ? [{
            Line: [{ _: supplier.address.street }],
          }] : undefined,
          Country: [{
            IdentificationCode: [{
        ...
        Contact: (supplier.phone || supplier.email) ? [{
          Telephone: supplier.phone ? [{ _: supplier.phone }] : undefined,
    ...
    return {
      Party: [{
        PartyIdentification: [
          {
            ID: [{
              _: customer.taxId || customer.code,
              schemeID: customer.taxId ? 'TIN' : 'BRN',
            }],
          },
        ],
        PostalAddress: [{
          CityName: customer.billingAddress?.city ? [{ _: customer.billingAddress.city }] : undefined,
          PostalZone: customer.billingAddress?.postalCode ? [{ _: customer.billingAddress.postalCode }] : undefined,
          CountrySubentityCode: customer.billingAddress?.state ? [{ _: customer.billingAddress.state }] : undefined,
          AddressLine: customer.billingAddress?.street ? [{
            Line: [{ _: customer.billingAddress.street }],
          }] : undefined,
          Country: [{
            IdentificationCode: [{
        ...
        Contact: (customer.phone || customer.email) ? [{
          Telephone: customer.phone ? [{ _: customer.phone }] : undefined,
