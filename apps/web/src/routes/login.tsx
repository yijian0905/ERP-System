...
function LoginPage() {
    ...
    // Use ref to track transition state synchronously (avoids race condition with Zustand)
    const isInTransitionRef = useRef(false);

    // Dashboard store for preloading
    const { fetchDashboardData, isLoaded: isDashboardLoaded } = useDashboardStore();

    // Redirect if already authenticated (but not if we're in transition)
    useEffect(() => {
        // Use ref for synchronous check since useState batching can be delayed
        if (isAuthenticated && !isInTransitionRef.current && !showTransition) {
            navigate({ to: redirect || '/dashboard' });
        }
    }, [isAuthenticated, navigate, redirect, showTransition]);
    ...
    const handleSubmit = useCallback(
        async (e: React.FormEvent) => {
            ...
            try {
                ...
                if (response.success && response.data) {
                    ...
                    // Login successful - continue with transition and preload
                    fetchDashboardData();
                    setAuth(user, accessToken, refreshToken);
                } else {
                    // Login failed - cancel transition and show error
                    isInTransitionRef.current = false;
                    setShowTransition(false);
                    setError(response.error?.message || 'Login failed. Please try again.');
                }
            } catch {
                // Error - cancel transition and show error
                isInTransitionRef.current = false;
                setShowTransition(false);
        ...
        [email, password, setAuth, fetchDashboardData]
    );
    ...
    return (
        <>
            ...
            <LoginTransition
                ...
                isDataLoaded={isDashboardLoaded}
            />
